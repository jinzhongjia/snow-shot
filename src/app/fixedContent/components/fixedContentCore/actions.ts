...
import { FixedContentProcessImageConfig, FixedContentType } from '.';
import { RefObject } from 'react';
import * as htmlToImage from 'html-to-image';
import { FixedContentCoreDrawActionType } from './components/drawLayer';
...
const applyRotationTransform = (
    ...
    if (processImageConfig.horizontalFlip || processImageConfig.verticalFlip) {
        ...
        const currentWidth = needSwapWidthAndHeight(angle) ? canvasHeight : canvasWidth;
        const currentHeight = needSwapWidthAndHeight(angle) ? canvasWidth : canvasHeight;
...
export const renderToCanvasAction = async (
    ...
    if (
        ...
        // 应用旋转和翻转变换
        applyRotationTransform(context, processImageConfigRef.current, canvas.width, canvas.height);
        ...
        const { angle, horizontalFlip, verticalFlip } = processImageConfigRef.current;
